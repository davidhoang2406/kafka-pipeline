...
| Job | File | Reads | Writes |
|---|---|---|---|
| `ohlcv_daily_ingest` | `analysis/batch/ohlcv_daily_ingest.py` | `s3a://market-data/price.snapshot/...avro` (one day) | `s3a://market-analysis/ohlcv.bar/...parquet` |
| `technical_job` | `analysis/batch/technical_job.py` | `s3a://market-analysis/ohlcv.bar/...` (full history) | `s3a://market-analysis/technical.indicators/...parquet` |
...
| Task | Command |
|---|---|
| Build image | `make spark-build` (or `make install` selecting Spark) |
| Start cluster | `docker compose up -d spark-master spark-worker spark-history-server` |
| Submit OHLCV ingest | `make run-ohlcv-daily-ingest [DATE=YYYY-MM-DD]` |
| Submit Technical job | `make run-spark-technical` |
| Logs | `docker logs spark-master` / `docker logs spark-worker` |
| Tear down (deletes event logs) | `make uninstall` then `y` for Spark |
...
- `analysis/batch/ohlcv_daily_ingest.py`, `analysis/batch/technical_job.py`
- `docker/spark.Dockerfile`
- `design/services/DAGSTER.md` — Phase 10 will orchestrate these jobs from outside the Spark cluster
