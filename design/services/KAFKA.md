...
| Task | Command |
|---|---|
| Start | `make install` (interactive) or `docker compose -f docker/docker-compose.yml up -d kafka kafka-ui` |
| Create topics | `make topics-create` (idempotent — uses `--if-not-exists`) |
...
| Tear down (destroys data) | `make uninstall` then answer `y` to Kafka |
