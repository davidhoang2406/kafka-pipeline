...
| Task | Command |
|---|---|
| Build image | `make jupyter-build` (first time or after `requirements.txt` changes) |
| Start | `make jupyter` → http://localhost:8888 |
| Logs | `docker logs jupyter` |
| Tear down | `make uninstall` then `y` for Jupyter |
...
- `model/spark.py` — `SparkFactory` (auto-selects local[*] in this container)
