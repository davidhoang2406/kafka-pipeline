...
| Service | Image | Ports |
|---|---|---|
| `minio` | `minio/minio:latest` | `9000:9000` (S3 API), `9001:9001` (Web console) |
...
| Task | Command |
|---|---|
| Start | `make install` or `docker compose -f docker/docker-compose.yml up -d minio` |
| Create buckets | `make minio-init` (runs `db/init_minio.py`; idempotent) |
...
| Flush a bucket | `make storage-flush` (interactive — asks per bucket) |
| Tear down (destroys data) | `make uninstall` then answer `y` to MinIO |
...
- `model/minio_store.py` — Python SDK wrapper (bucket init, Avro/Parquet I/O, lifecycle rules)
- `db/init_minio.py`, `db/flush_minio.py`
- `model/spark.py` — S3A connector configuration on SparkSession
