...
**Status:** Design draft · Branch `feature/phase-10-orchestration-planning`
**Author:** David Hoang
**Date:** 2026-05-16
...
```
Kafka/
├── orchestration/                      # NEW — Dagster code location
│   ├── pyproject.toml                  # dagster-cloud-cli + deps
│   ├── dagster_project/
│   │   ├── __init__.py                 # exports `defs: Definitions`
│   │   ├── assets/
│   │   │   ├── price_snapshots.py      # SourceAsset
│   │   │   ├── ohlcv.py                # @asset ohlcv_daily_bars
│   │   │   ├── technical.py            # @asset technical_indicators
│   │   │   ├── digest.py               # Phase 11
│   │   │   └── screener.py             # Phase 11
...
│       └── test_assets.py
├── docker/
│   └── dagster.Dockerfile              # NEW — webserver + daemon image
├── docker/docker-compose.yml           # MODIFIED — add dagster service
└── Makefile                            # MODIFIED — add make airflow targets:
                                        #   make orchestration-up
                                        #   make orchestration-down
                                        #   make orchestration-shell
```
...
```bash
# One-time
make orchestration-build           # build dagster.Dockerfile
make orchestration-up              # start webserver + daemon → http://localhost:3000

# Day-to-day
# - Edit assets in orchestration/dagster_project/
