...
def price_snapshots(context: AssetExecutionContext, minio: MinioResource) -> ObserveResult:
    """Observe whether today's price snapshot partition exists in MinIO market-data."""
    date       = context.partition_key          # "2026-05-16"
    year, month, day = date[:4], date[5:7], date[8:10]

    # Check for at least one stock file in today's partition (crypto may not trade on weekends)
